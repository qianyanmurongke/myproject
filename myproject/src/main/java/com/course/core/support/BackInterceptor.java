...
import com.course.core.security.ShiroUser;
...
public class BackInterceptor implements HandlerInterceptor {
	...
	public static final String ORG_KEY = "_org";

	/**
	 * Controller执行前执行。判断用户是否登录，将用户信息通过 {@link Context#setCurrentUser(User)}
	 * 保存在线程变量中，方便在其他地方获取。
	 */
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if (shiroUser != null) {
			...
			resolveOrg(request, response);
		}
		return true;
	}

	private void resolveOrg(HttpServletRequest request, HttpServletResponse response) {
		Org org = null;
		Integer orgId;
		// 从parameter中获取
		String orgIdText = request.getParameter(ORG_KEY);
		if (StringUtils.isNotBlank(orgIdText)) {
			try {
				orgId = Integer.parseInt(orgIdText);
				org = this.orgService.get(orgId);
			} catch (Exception e) {
				// continue
			}
		}
		// 从cookie中获取
		if (org == null) {
			orgIdText = Servlets.getCookie(request, ORG_KEY);
			if (StringUtils.isNotBlank(orgIdText)) {
				try {
					orgId = Integer.parseInt(orgIdText);
					org = this.orgService.get(orgId);
		...
		if (org == null) {
			org = Context.getCurrentOrg(request);
		} 
		
		addOrgCookie(org.getId().toString(), request, response);

		Context.setCurrentOrg(request, org);

	}

	private void addOrgCookie(String orgIdText, HttpServletRequest request, HttpServletResponse response) {
		Cookie cookie = new Cookie(ORG_KEY, orgIdText);
	...
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler,
		...
		if (!isRedirect) {
			...
			modelMap.addAttribute("admin", Constants.CMSCP);
