...
import com.course.common.security.IncorrectCaptchaException;
...
public class CmsAuthenticationFilter extends FormAuthenticationFilter {
	...
	protected boolean executeLogin(ServletRequest request, ServletResponse response) throws Exception {
		...
		if (user != null) {
			String rawPassword = this.getCredentialsDigest().digest(this.getPassword(request), user.getSaltBytes());
			rawPassword = rawPassword == null ? "" : rawPassword;
			token = createToken(username, rawPassword, request, response);
		...
		try {
			...
			getOperationLogService().loginSuccess(ip, user.getId(), userAgent);
			// 清除学校Cookie
			removeOrgCookie(hsr, hsp);
			...
			hsr.getSession().setAttribute(WebUtils.SAVED_REQUEST_KEY, savedRequest);
			String userAgent = hsr.getHeader("user-agent");
			getOperationLogService().loginFailure(username + ":" + password, ip, userAgent);
	...
	private void removeOrgCookie(HttpServletRequest request, HttpServletResponse response) {
		...
		for (Cookie cookie : cookies) {
			if (cookie.getName().equals(ORG_KEY)) {
	...
	public CredentialsDigest getCredentialsDigest() {
		if (credentialsDigest == null) {
			credentialsDigest = beanFactory.getBean(CredentialsDigest.class);
		}
