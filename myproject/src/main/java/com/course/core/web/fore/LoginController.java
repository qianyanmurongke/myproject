...
import static com.course.core.security.CmsAuthenticationFilter.FALLBACK_URL_PARAM;
...
public class LoginController {
	...
	@RequestMapping(value = { LOGIN_URL, Constants.SITE_PREFIX_PATH + "" + LOGIN_URL })
	public String login(String fallbackUrl, HttpServletRequest request, org.springframework.ui.Model modelMap) {

		HttpSession session = request.getSession();
		session.setAttribute(CmsAuthenticationFilter.CAPTCHA_REQUIRED_KEY, true);
	...
	@RequestMapping(value = { "/login_include", Constants.SITE_PREFIX_PATH + "/login_include" })
	public String loginInclude(String fallbackUrl, HttpServletRequest request, HttpServletResponse response,
	...
	@RequestMapping(value = { "/login_ajax", Constants.SITE_PREFIX_PATH + "/login_ajax" })
	public String loginAjax(String fallbackUrl, HttpServletRequest request, HttpServletResponse response,
			org.springframework.ui.Model modelMap) {
		Site site = Context.getCurrentSite();
		Map<String, Object> data = modelMap.asMap();
		ForeContext.setData(data, request);
	...
	@RequestMapping(value = { "/login", Constants.SITE_PREFIX_PATH + "/login" }, method = RequestMethod.POST)
	public String loginFail(@RequestParam(DEFAULT_USERNAME_PARAM) String username, String fallbackUrl,
