...
import com.course.common.web.DateEditor;
import com.course.common.web.StringEmptyEditor;
import com.course.common.web.TimerInterceptor;
import com.course.core.constant.Constants;
import com.course.core.support.ForeInterceptor;
import com.course.core.support.JsonInterceptor;
import com.course.core.support.SitePhoneInterceptor;

/**
 * 应用启动的主入口
 */
@Configuration
// 此三项也未用到，但二次开发时有可能会用到，暂时不屏蔽：JdbcTemplateAutoConfiguration.class,
// JtaAutoConfiguration.class,
// WebSocketAutoConfiguration.class
// 暂时不用Mobile的自动配置：DeviceResolverAutoConfiguration.class,
...
@ImportResource({ "classpath:conf/**/context*.xml", "classpath:custom.xml","classpath:conf/quartz-task.xml"})
...
public class Application extends SpringBootServletInitializer implements WebApplicationInitializer {
	...
	/**
	 * 注册后台DispatcherServlet。只处理`/admin/*`相关请求
	 * 
	...
	public ServletRegistrationBean backendDispatcherServletRegistration(
		...
		ServletRegistrationBean registration = new ServletRegistrationBean(backendDispatcherServlet(), "/admin/*");
		// 必须指定启动优先级，否则无法生效
	...
	protected static class WebConfig extends WebMvcConfigurerAdapter {
		...
		@Bean
		public SitePhoneInterceptor SitePhoneInterceptor() {
			return new SitePhoneInterceptor();
		}

		@Override
		public void addInterceptors(InterceptorRegistry registry) {
			registry.addInterceptor(new TimerInterceptor());
			registry.addInterceptor(foreInterceptor()).excludePathPatterns("/error/**").excludePathPatterns("/api/**")
					.excludePathPatterns("/site-phone/**");
			registry.addInterceptor(JsonInterceptor()).addPathPatterns("/api/**").addPathPatterns("/site-classcard/**");
			registry.addInterceptor(SitePhoneInterceptor()).addPathPatterns("/site-phone/**");
