package com.course.common.image;
...
public class ImageHelper {
	 ...
	 *            图图片格式 例如 jpg
	 */
	public static String scaleImageWithParams(String sourceImagePath, String destinationPath, int width, int height,
			boolean auto) {
		...
		try {
			...
			String filePrex = sourceImagePath.substring(0, sourceImagePath.indexOf('.'));
	 ...
	 *            图图片格式 例如 jpg
	 */
	public static String scaleImageWithParams(String sourceImagePath, int width, int height, boolean auto,
			String format) {
		String path = "D:\\Tomcat 8.0\\webapps\\ROOT\\";
		...
		try {
			...
			String filePrex = sourceImagePath.substring(0, sourceImagePath.indexOf('.'));
			/** 压缩后的文件名 */
			destinationPath = filePrex + "_" + width + "_" + sourceImagePath.substring(filePrex.length());

			if (new File(path + destinationPath).exists()) {
				return destinationPath;
			}
			...
			/** 压缩之后临时存放位置 */
			FileOutputStream out = new FileOutputStream(path + destinationPath);
			JPEGImageEncoder encoder = JPEGCodec.createJPEGEncoder(out);
			JPEGEncodeParam jep = JPEGCodec.getDefaultJPEGEncodeParam(outputImage);
			/** 压缩质量 */
			jep.setQuality(1f, true);
