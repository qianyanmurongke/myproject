...
public class Images {
	...
	public static BufferedImage crop(final BufferedImage buff, int rorate, int x, int y, int width, int height) {
		Graphics2D g = (Graphics2D) buff.getGraphics();
		g.rotate(rorate);
		if (rorate != 0) {
			rotateImage(buff, rorate);
		}

		return crop(buff, x, y, width, height);
	}
