...
public class JpqlBuilder {
	...
	public JpqlBuilder setParameters(List<Parameter> parameters) {
	...
	public Query createCountQuery(EntityManager em) {
		String cqs = StringUtils.isNotBlank(getCountQueryString()) ? getCountQueryString()
				: QueryUtils.createCountQueryFor(jpql.toString(), getCountProjection());
	...
	public Query createNativeCountQuery(EntityManager em) {
		String cqs = StringUtils.isNotBlank(getCountQueryString()) ? getCountQueryString() : createNativeCountQueryString();
	...
	public String createNativeCountQueryString() {
		...
		int end = -1;//groupStart == -1 ? orderStart : groupStart;
	...
	@SuppressWarnings("rawtypes")
	public List listNative(EntityManager em, Sort sort) {
		Query query = this.createNativeQuery(em, sort);
	...
	@SuppressWarnings("rawtypes")
	public Page nativePageByMap(EntityManager em, Pageable pageable) {
		Query countQuery = this.createNativeCountQuery(em);
		long total = ((Number) countQuery.getSingleResult()).longValue();
