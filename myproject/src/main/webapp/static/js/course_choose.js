...
Cms.F7SingleDialog = function(url, name, names, focus, options) {
	options = options || {};
	var idField = $("#" + name);
	var f7Button = $("#" + name + "Button");
	...
	f7Button.click(function() {
		...
		BootstrapDialog.show({
			...
			cssClass: 'choosee-dialog',
			message: $('<div style="width:300px;height:350px;position: relative;overflow: auto;"></div>').load(url + "?" + $.param(params)),
			buttons: [{
				label: '<i class="glyphicon glyphicon-ok"></i>&nbsp;选择',
...
Cms.f7.perm = function(name, options) {
	...
	options.settings = $.extend(settings, options.settings);
	var names = ["Number"];
	var f7 = Cms.F7SingleDialog(url, name, names, name, options);
};
/**
 * 信息权限多选
 */
...
Cms.f7.org = function(name, focus, options) {
	...
	options.settings = $.extend(settings, options.settings);
	var names = ["Number", "Name"];
	var f7 = Cms.F7SingleDialog(url, name, names, focus, options);
};
/**
 * 组织多选
 */
 ...
 * 部门单选
 */
Cms.f7.department = function(name, focus, options) {
	options = options || {};
	var url = CTX + CMSCP + "/education/department/choose_department_tree.do";
	var settings = {
 ...
 * 部门多选
 */
Cms.f7.departmentMulti = function(name, options) {
	options = options || {};
	var url = CTX + CMSCP + "/education/department/choose_department_tree_multi.do";
	var settings = {
 ...
 * 章节单选
 */
Cms.f7.chapter = function(name, focus, options) {
	options = options || {};
	var url = CTX + CMSCP + "/education/chapter/choose_chapter_tree.do";
	var settings = {
