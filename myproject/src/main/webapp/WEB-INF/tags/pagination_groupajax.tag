...
<%@ attribute name="formId" type="java.lang.String" required="true" rtexprvalue="true"%>

<ul class="pagination pull-left" style="margin:0;">
  <li<c:if test="${fnx:bool(pagedList,'isFirst')}"> class="disabled"</c:if>>
    <a onclick="<c:if test="${!fnx:bool(pagedList,'isFirst')}">$('#${formId}page').val('${pagedList.number}');$('#${formId}').submit();</c:if>return false;" aria-label="上一页">
      <span aria-hidden="true">«</span>
    </a>
  </li>
  <li<c:if test="${fnx:bool(pagedList,'isFirst')}"> class="active"</c:if>>
    <a onclick="<c:if test="${!fnx:bool(pagedList,'isFirst')}">$('#${formId}page').val('1');$('#${formId}').submit();</c:if>return false;">
  ...
  <c:if test="${end>=begin}">
	  ...
	  <li<c:if test="${pagedList.number+1==i}"> class="active"</c:if>>
	    <a onclick="<c:if test="${!(pagedList.number+1==i)}">$('#${formId}page').val('${i}');$('#${formId}').submit();</c:if>return false;">
	      <span>${i}</span>
	    </a>
	  </li>
	  </c:forEach>
  ...
  <li<c:if test="${pagedList.number+1 == pagedList.totalPages}"> class="active"</c:if>>
    <a   onclick="debugger;$('#${formId}page').val('${pagedList.totalPages}');$('#${formId}').submit();return false;">
  ...
  <li<c:if test="${fnx:bool(pagedList,'isLast')}"> class="disabled"</c:if>>
    <a onclick="<c:if test="${pagedList.number+2<=pagedList.totalPages}">$('#${formId}page').val('${pagedList.number+2}');$('#${formId}').submit();</c:if>return false;" aria-label="下一页">
      <span aria-hidden="true">»</span>
    </a>
  </li>
</ul>
<input type="hidden" id="${formId}page" name="page" value="${pagedList.number+1}"/>
<div class="pull-left" style="padding:0 6px;">
	&nbsp;每页
	<select class="form-control" name="page_size" id="pageSize" onchange="$.cookie('page_size',$(this).val(),{expires:36500});$('#${formId}').submit();" style="border-radius:4px;padding-left:3px;padding-right:3px;width:60px;display:inline-block;">
...
$(function(){
	...
	$("#${formId}page").val(${pagedList.totalPages});
	</c:if>
});
</script>