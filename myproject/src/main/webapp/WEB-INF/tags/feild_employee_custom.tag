<%@ tag pageEncoding="UTF-8"%><%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%><%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%><%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions"%><%@ taglib prefix="fnx" uri="http://java.sun.com/jsp/jstl/functionsx"%><%@ taglib prefix="s" uri="http://www.springframework.org/tags" %><%@ taglib prefix="f" uri="http://www.course.com/tags/form"%><%@ taglib prefix="tags" tagdir="/WEB-INF/tags"%>
...
<%@ attribute name="prefixname" type="java.lang.String" required="true" rtexprvalue="true"%>
  <c:choose>
    <c:when test="${field.innerType==3}">
      <c:set var="c_name" value="${field.name}"/>
    </c:when>
    <c:otherwise>
    	<c:set var="c_name"><c:out value="${(field.clob) ? ('clobs_') : (prefixname)}${field.name}"/></c:set>
    </c:otherwise>
  </c:choose>
  <c:choose>
    <c:when test="${field.innerType==3}">
    ...
    <c:when test="${field.type==4 || field.type==100}">
      ...
      var ${c_name}SwfUpload = Cms.swfUploadFile("${c_name}",{
        ...
        file_types: "${GLOBAL.upload.fileTypes}"
 		...
 		<c:when test="${field.type==50}">
		  ...
		  function create_ueditor_${field.name}(content) {
	        ...
	        serverUrl:"${ctx}${admin}/core/ueditor.do?ueditor=true"
		    });
		  ...
		  function create_editormd_${field.name}(markdown) {
			  ...
			  editormd_${field.name} = editormd("${c_name}", {
				  ...
				  <c:choose>
					...
					imageUploadURL : "${ctx}${admin}/core/upload_image.do?editormd=true",
					...
					onload : function() {
						base64UploadPlugin(this, "${ctx}${admin}/core/upload_image.do");
					}
				});
		  }
	    ...
	    $(function() {
          ...
          <c:if test="${!empty field.customs['width']}">initialFrameWidth:${field.customs['width']},</c:if>
          <c:if test="${!empty field.customs['height']}">initialFrameHeight:${field.customs['height']},</c:if>
	        imageUrl: "${ctx}${admin}/core/upload_image.do?ueditor=true",
	        wordImageUrl: "${ctx}${admin}/core/upload_image.do?ueditor=true",
	        fileUrl: "${ctx}${admin}/core/upload_file.do;jsessionid=<%=request.getSession().getId()%>?ueditor=true",
	        videoUrl: "${ctx}${admin}/core/upload_video.do;jsessionid=<%=request.getSession().getId()%>?ueditor=true",
	        catcherUrl: "${ctx}${admin}/core/get_remote_image.do?ueditor=true",
	        imageManagerUrl: "${ctx}${admin}/core/image_manager.do",
	        getMovieUrl: "${ctx}${admin}/core/get_movie.do",
