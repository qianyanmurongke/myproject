								...
								<div class="form-group">
									...
									<div class="col-sm-8">
										<c:set var="parentName">
											<c:choose>
												<c:when test="${empty parent}"><s:message code="site.root" /></c:when>
												<c:otherwise>
													<c:out value="${parent.displayName}" /></c:otherwise>
											</c:choose>
										</c:set>
										...
										<script type="text/javascript">
											$(function() {
												Cms.f7.site("parentId", "parentIdName", {
													"settings": {
														"title": "<s:message code='site.f7.selectSite'/>"
													},
													"params": {
