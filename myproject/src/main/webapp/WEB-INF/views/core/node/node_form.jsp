				  	...
				  	<div class="form-group">
						  ...
						  <c:when test="${field.name eq 'infoPerms'}">
						  	...
						  	<c:forEach var="role" items="${roleList}">
									<label class="checkbox-inline"><input type="checkbox" name="infoPermIds" value="${role.id}"<c:if test="${fnx:contains_co(bean.infoPerms,role) || role.allInfoPerm || empty bean}"> checked</c:if>/>${role.name}</label>
						  	</c:forEach>
						  </c:when>
						  <c:when test="${field.name eq 'nodePerms'}">
						  	<input type="hidden" name="nodePermIdsExist" value="true"/>
						  	<c:forEach var="role" items="${roleList}">
									<label class="checkbox-inline"><input type="checkbox" name="nodePermIds" value="${role.id}"<c:if test="${fnx:contains_co(bean.nodePerms,role)}"> checked</c:if>/>${role.name}</label>
						  	</c:forEach>
						  </c:when>
