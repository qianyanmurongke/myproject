		...
		<form class="form-horizontal" id="validForm" action="${oprt=='edit' ? 'update' : 'save'}.do" method="post">
			...
			<div class="box-header with-border">
				<div class="btn-toolbar">
			      ...
			      <button style="display: none;" class="btn btn-default" type="button" onclick="window.open('${bean.url}');"<c:if test="${oprt=='create'||bean.status ne 'A'}"> disabled="disabled"</c:if>><s:message code="info.foreView"/></button>
					</div>
					<div class="btn-group">
			  ...
			  <div class="row">
				  ...
				  <div class="col-sm-${field.dblColumn?'6':'12'}">
				  	<div class="form-group">
						  ...
						  <c:when test="${field.name eq 'classes'}">
						  	<select name="customs_classesid" class="form-control">
					  		 	<c:forEach var="classes" items="${classlist}">
						  		<f:option value="${classes.id}" selected="${bean.customs['classesid']}">${classes.gradeClassName}</f:option>
						  		</c:forEach>
						  	</select>
						  </c:when>
